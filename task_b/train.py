"""
Training script for Task B: Authorship Attribution (Cascade)

Trains cascade classifier with two stages:
1. Binary: Human vs AI
2. Family: 10 AI families (with meta-learning)
"""
...
try:
    from task_b.model import TaskBModel
...
class TaskBTrainer:
    """Trainer for Task B: Authorship Attribution (Cascade)."""
    ...
    def __init__(self, config_path: str, resume_from: Optional[str] = None):
        ...
        # Train in cascade: first binary, then family
        self.stage = 'binary'  # 'binary' or 'family'
        ...
        self.train_dataset = TaskBDataset(
        ...
        # Create dataloaders
        self.train_loader = create_task_b_dataloader(
            self.train_dataset,
            batch_size=self.config['training']['batch_size'],
            shuffle=True,
            num_workers=self.config['data'].get('num_workers', 0)
        )
        self.val_loader = create_task_b_dataloader(
            self.val_dataset,
    ...
    def switch_stage(self, stage: str):
        """Switch between binary and family stages."""
        self.stage = stage
        print(f"\n{'='*60}")
    ...
    def train_epoch(self, epoch: int):
        ...
        num_classes = 2 if self.stage == 'binary' else 11
    ...
    def train(self):
        ...
        print("\n" + "=" * 60)
        print("Stage 1: Binary Classification (Human vs AI)")
        print("=" * 60)
        best_binary_f1 = self.train_stage('binary', self.config['training']['binary_epochs'])
        
        # Stage 2: Family (10 AI families)
        print("\n" + "=" * 60)
        print("Stage 2: Family Classification (10 AI families)")
        print("=" * 60)
        best_family_f1 = self.train_stage('family', self.config['training']['family_epochs'])
        ...
        print(f"✓ Best family F1: {best_family_f1:.4f}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train Task B model (cascade)")
    parser.add_argument(
        '--config',
        type=str,
        default='task_b/config.yaml',
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Path to checkpoint to resume from (e.g., checkpoints/task_b/binary/best_model.pt)'
    )
    args = parser.parse_args()
    
    # Train
    trainer = TaskBTrainer(args.config, resume_from=args.resume)
    trainer.train()
